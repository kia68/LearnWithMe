...
class JobQueueImpl(private val jobRepository: JobRepository) : JobQueue {
    ...
    @Transactional
    override fun enqueue(type: JobType, jobKey: String, payload: Map<String, Any?>): UUID {
        jobRepository.findByJobKey(jobKey)?.let { return it.id }
        val job = jobRepository.save(
            JobEntity(jobKey = jobKey, type = type, payload = objectMapper.writeValueAsString(payload)),
        )
        return job.id
    }
}
